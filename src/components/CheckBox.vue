<template>
  <input
    :id="`checkboxId${checkboxId}`"
    class="checkbox-input"
    type="checkbox"
    :value="itemChecked"
    @change="$emit('editToDoItem', $event.target.value)"
  />
  <label :for="`checkboxId${checkboxId}`">
    <div>
      <div v-if="itemChecked" class="checkbox-container">
        <svg
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <g opacity="0.9">
            <path
              class="checked-on-hover"
              d="M11.9801 19.4262C16.0726 19.4262 19.3903 16.1085 19.3903 12.016C19.3903 7.92339 16.0726 4.60571 11.9801 4.60571C7.8875 4.60571 4.56982 7.92339 4.56982 12.016C4.56982 16.1085 7.8875 19.4262 11.9801 19.4262Z"
              fill="#5E60CE"
            />
            <path
              class="checked-on-hover"
              d="M11.9643 4.85959C13.3765 4.85959 14.7571 5.27837 15.9313 6.06298C17.1056 6.84758 18.0208 7.96278 18.5612 9.26753C19.1017 10.5723 19.2431 12.008 18.9676 13.3931C18.6921 14.7782 18.012 16.0506 17.0134 17.0492C16.0148 18.0478 14.7424 18.7279 13.3573 19.0034C11.9722 19.2789 10.5365 19.1375 9.23173 18.597C7.92697 18.0566 6.81178 17.1414 6.02717 15.9671C5.24257 14.7929 4.82378 13.4123 4.82378 12.0001C4.8311 10.1086 5.58574 8.29659 6.92326 6.95907C8.26079 5.62155 10.0728 4.8669 11.9643 4.85959ZM11.9643 3.27281C10.2397 3.27986 8.55592 3.7977 7.12543 4.76094C5.69494 5.72419 4.58191 7.08965 3.92683 8.68496C3.27176 10.2803 3.10401 12.0339 3.44478 13.7245C3.78554 15.415 4.61954 16.9667 5.84148 18.1837C7.06342 19.4007 8.61852 20.2283 10.3105 20.5622C12.0024 20.896 13.7553 20.7211 15.348 20.0596C16.9406 19.398 18.3015 18.2794 19.2589 16.845C20.2163 15.4106 20.7272 13.7246 20.7273 12.0001C20.7273 10.851 20.5003 9.71321 20.0595 8.65203C19.6187 7.59086 18.9727 6.62718 18.1585 5.81631C17.3443 5.00544 16.378 4.36334 15.315 3.92686C14.2521 3.49038 13.1134 3.26811 11.9643 3.27281Z"
              fill="#5E60CE"
            />
            <path
              d="M15.4306 9.34215L11.0986 13.6741L8.61618 11.1916L7.78027 12.0275L11.0986 15.3459L16.2665 10.1781L15.4306 9.34215Z"
              fill="#F2F2F2"
            />
          </g>
        </svg>
      </div>
      <div v-if="!itemChecked" class="checkbox-container">
        <svg
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
          stroke="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            class="unchecked-on-hover"
            d="M11.9643 4.85958C13.3765 4.85958 14.7571 5.27836 15.9313 6.06297C17.1056 6.84758 18.0208 7.96277 18.5612 9.26752C19.1017 10.5723 19.2431 12.008 18.9676 13.3931C18.6921 14.7782 18.012 16.0505 17.0134 17.0492C16.0148 18.0478 14.7424 18.7278 13.3573 19.0034C11.9722 19.2789 10.5365 19.1375 9.23173 18.597C7.92697 18.0566 6.81178 17.1414 6.02717 15.9671C5.24257 14.7929 4.82378 13.4123 4.82378 12.0001C4.8311 10.1085 5.58574 8.29658 6.92326 6.95906C8.26079 5.62154 10.0728 4.86689 11.9643 4.85958ZM11.9643 3.2728C10.2397 3.27986 8.55592 3.79769 7.12543 4.76094C5.69494 5.72418 4.58191 7.08965 3.92683 8.68495C3.27176 10.2803 3.10401 12.0339 3.44478 13.7245C3.78554 15.415 4.61954 16.9667 5.84148 18.1837C7.06342 19.4007 8.61852 20.2283 10.3105 20.5622C12.0024 20.896 13.7553 20.7211 15.348 20.0595C16.9406 19.398 18.3015 18.2794 19.2589 16.845C20.2163 15.4105 20.7272 13.7246 20.7273 12.0001C20.7273 10.851 20.5003 9.7132 20.0595 8.65202C19.6187 7.59085 18.9727 6.62717 18.1585 5.8163C17.3443 5.00543 16.378 4.36333 15.315 3.92685C14.2521 3.49037 13.1134 3.2681 11.9643 3.2728Z"
            fill="#4EA8DE"
          />
        </svg>
      </div>
    </div>
  </label>
</template>

<script setup>
import { defineEmits, defineProps } from "vue";
defineProps(["checkboxId", "itemChecked"]);
defineEmits(["editToDoItem"]);
</script>

<style scoped>
.checkbox-input {
  overflow: hidden;
  visibility: hidden;
  padding: 0;
}

.checkbox-container {
  padding: 0;
  margin: 0;
  width: 24px;
  height: 24px;
}

.checkbox-container > div {
  height: 24px;
}

svg:hover path.checked-on-hover {
  fill: #8284fa;
}

svg:hover path.unchecked-on-hover {
  fill: #1e6f9f;
}
</style>
